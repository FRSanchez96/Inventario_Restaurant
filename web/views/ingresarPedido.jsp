...
<body>
	...
	<div id="sidebar" class="nav-collapse ">
		...
		<ul class="sidebar-menu" id="nav-accordion">
			...
			<p class="centered">
				<a href="gestionarproductos.jsp"><img src="../assets/img/ui-sam.jpg"
					class="img-circle" width="60"></a>
			</p>
			<h5 class="centered">Gestion de productos</h5>
...
<%
    ...
    FacturaDAO bq = new FacturaDAO();
    PedidosDAO ks = new PedidosDAO();
    BodegaDAO pk = new BodegaDAO();
    Tipo_PagoDAO qt = new Tipo_PagoDAO();
    ArrayList<Usuario> tr = pk.LUsu();
    ArrayList<Mesa> me = ks.ListMesa();
    ArrayList<Tipo_Pago> gf = qt.ListTip_Pago();
    ArrayList<Empleado> jh = ks.LisEmpleado();
    ArrayList<Proveedor> kr = pk.LProve();
%>
	<form action="../Cocina" method="post">
		<table border="1">
                    
                    <h3>Ingresar Pedido</h3>
                    <h4>Bienvenido en esta seccion puede ingresar un pedido</h4>
                        <tr>
				<td>Monto Pedido:</td>
                                <td><input type="number"  name="montoP" value="" ></td>
                        </tr>
                        <tr>
				<td>Descripción Boleta:</td>
                                <td><input type="text"  name="descB" value="" size="50"></td>
                        </tr>
                        <tr>
				<td>Rut del Trabajador:</td>
                                ...
                                <td><select name="nTa">
                                <%
                                     for(Proveedor tmx:kr)
                                        {
                                %>
                                        <option value="<%=tmx.getRut_proveedor()%>"><%=tmx.getRut_proveedor()%></option>
                                <%
                                        }
                                %>
                                </select>
				...
				<td>Numero Trabajador:</td>
                                ...
                                <%
                                     for(Empleado tmx:jh)
                                        {
                                %>
                                        <option value="<%=tmx.getID_EMP()%>"><%=tmx.getRUT()%></option>
                                <%
                                        }
                                %>
                                </select>
				...
				<td>Nombre del Trabajador:</td>
                                ...
                                <%
                                     for(Usuario tmx:tr)
                                        {
                                %>
                                        <option value="<%=tmx.getRUT()%>"><%=tmx.getNOMBRE()%> <%=tmx.getAPELLIDO_PATERNO()%> <%=tmx.getAPELLIDO_MATERNO()%></option>
                                <%
                                        }
                                %>
                                </select>
				...
				<td>N° Mesa:</td>
                                ...
                                <%
                                     for(Mesa tmx:me)
                                        {
                                %>
                                        <option value="<%=tmx.getNUMERO_MESA()%>"><%=tmx.getNUMERO_MESA()%></option>
                                <%
                                        }
                                %>
                                </select>
				...
				<td>Tipo de Pago:</td>
                                ...
                                <td><select name="tPa">
                                <%
                                     for(Tipo_Pago tmx:gf)
                                        {
                                %>
                                        <option value="<%=tmx.getId_tipo_pago()%>"><%=tmx.getDescripcion()%></option>
                                <%
                                        }
                                %>
                                </select>
        ...
        <br> 
            <input type="submit" class="btn btn-primary" name="opcion" value="Ingresar Pedido">
