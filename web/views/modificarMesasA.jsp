...
<%
 ...
 %>    
	...
	<h3>Modificación de Mesas</h3>
	<h4>Bienvenido en esta seccion puede modificar sus mesas</h4>
<%
    int valor = Integer.parseInt(request.getParameter("Id_BO"));
    
    PedidosDAO qq = new PedidosDAO();
    ArrayList<Mesa> aa = qq.ListMesa();
    ArrayList<Empleado> bb = qq.LisEmpleado();
    ArrayList<Reserva> cc = qq.ListReserva();
...
%>
	<form action="../Admin" method="post">
		<table border="1">
                    <%
                        for(Mesa tmp:aa)
                        {
                            for(Empleado tmx:bb) {
                             for(Reserva tmb:cc){
                                 if(tmp.getNUMERO_MESA()==valor){
                                if(tmp.getID_EMP()==tmx.getID_EMP()){
                                    if(tmp.getID_RESERVA()==tmb.getID_RESERVA()){
                            
                    %>
                        <tr>
                                <td>Número de la mesa:</td>
                                <td><input type="number" readonly="readonly" value="<%=tmp.getNUMERO_MESA()%>" name="idM"></td>
                        </tr>
			<tr>
				<td>Estado de la Mesa:</td>
                        ...
                        <tr>
				<td>Cantidad de Sillas:</td>
                                <td><input type="number" name="cantidadS" value="<%=tmp.getCANTIDAD_SILLA()%>"></td>
			</tr>
                     <%             }
                                  }
                                    }
        ...
        <br> 
        <input type="submit" class="btn btn-primary" name="opcion" value="Modificar Mesa">
