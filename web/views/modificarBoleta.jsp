...
%> <!--sideband--> <!-- **********************************************************************************************************************************************************
	...
	<h3>Modificación de Boleta</h3>
	<h4>Bienvenido en esta seccion puede modificar sus boletas</h4>
<%
    int valor = Integer.parseInt(request.getParameter("Id_BO"));
    
    BoletaDAO bq = new BoletaDAO();
    Tipo_PagoDAO qq =  new Tipo_PagoDAO();
    ArrayList<Boleta> Lbd = bq.buscarBoleta(valor);
    ArrayList<Tipo_Pago> lpq = qq.ListTip_Pago();
	...
	<form action="../Boleta" method="post">
		<table border="1">
                    ...
                    %>
                        <tr>
                                <td>Id Boleta</td>
                                <td><input type="number" readonly="readonly" value="<%=tmp.getID_BOLETA()%>" name="idbole"></td>
                        </tr>
			<tr>
				<td>Tipo de Pago:</td>
                                <td><select name="tipoPago">
                                    <%
                                        for(Tipo_Pago tmx:lpq)
                                        {
                                            //if(tmp.getTIPO_PAGO().equalsIgnoreCase(tmx.getDescripcion())){
                                    %>
                                    <option value="<%=tmx.getDescripcion()%>"><%=tmx.getDescripcion()%></option>
                                    <%        
                                           // }
                                        }
                                    %> 
			...
			<tr>
				...
				<td><input type="text" name="detPedido" value="<%=tmp.getDETALLE_PEDIDO()%>" size="50"></td>
			</tr>
                        <tr>
				<td>Número del Pedido:</td>
                                <td><input type="number" name="numPedido" value="<%=tmp.getID_PEDIDO()%>"></td>
			</tr>
                        <tr>
				<td>Número de la Transacción:</td>
                                <td><input type="text" name="numTrans" value="<%=tmp.getNUMERO_TRANSACCION()%>"></td>
			</tr>
                        <tr>
				<td>Total de la Boeta:</td>
                                <td><input type="number" name="Tboleta" value="<%=tmp.getTOTAL_BOLETA()%>"></td>
			</tr>
                     <%
                         }
                     %>
        ...
        <br> 
        <input type="submit" class="btn btn-primary" name="opcion" value="Modificar Boleta">
